...
while($fac = mysqli_fetch_assoc($faculty))
{
	...
	$courses = mysqli_query($conn,"SELECT cf.* FROM `kiusc_course_offered` cf join kiusc_evaluation e on cf.sem_id = e.sem_id WHERE cf.fac_id = '".$fac['id']."' and e.id = '$evl_id' and cf.eva_cat_id !=5");
