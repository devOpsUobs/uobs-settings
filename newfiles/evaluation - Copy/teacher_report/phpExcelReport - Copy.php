...
for($i=0; $i<count($cat_names); $i++)
{
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, round($cat_obt[$i]/$cat_total[$i]*100,2));
	$row++;
	$objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, $row, $cat_total[$i]);
	$row--;
	$col++;
	
	$ful_total += $cat_total[$i];
	$ful_obt += $cat_obt[$i];
}
...
header('Location: downloadFile.php?d=ddd'.$dep['name']);
