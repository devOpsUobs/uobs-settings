...
?>
	...
	<script type="text/javascript">
			...
			$('#dg').datagrid({
				...
				onExpandRow: function(index,row){
					...
					ddv.panel({
						border:false,
						cache:true,
						href:'<?php echo JURI::root( true ) ?>/newfiles/admin/admin_department/form_department.php?index='+index,
						onLoad:function(){
		...
		function saveItem(index){
			...
			var url = row.isNewRecord ? '<?php echo JURI::root( true ) ?>/newfiles/admin/admin_department/save_department.php' : '<?php echo JURI::root( true ) ?>/newfiles/admin/admin_department/update_department.php?id='+row.id;
			...
			if (row){
				$.messager.confirm('Confirm','Are you sure you want to remove this department?',function(r){
					if (r){
						...
						$.post('<?php echo JURI::root( true ) ?>/newfiles/admin/admin_department/delete_department.php',{id:row.id},function(s){
...
<body>
	<div class="demo-info" style="margin-bottom:10px">
		...
		<div>Manage Admin Departments</div>
	</div>
	
	
	<table id="dg" title="My Users" style="width:900px;height:450px"
			url="<?php echo JURI::root( true ) ?>/newfiles/admin/admin_department/get_departments.php"
