...
?>
	...
	<script type="text/javascript">
			...
			$('#dg').datagrid({
				...
				onExpandRow: function(index,row){
					...
					ddv.panel({
						border:false,
						cache:true,
						href:'<?php echo JURI::root( true ) ?>/newfiles/admin/department/form_department.php?index='+index,
						onLoad:function(){
		...
		function saveItem(index){
			...
			var url = row.isNewRecord ? '<?php echo JURI::root( true ) ?>/newfiles/admin/department/save_department.php' : '<?php echo JURI::root( true ) ?>/newfiles/admin/department/update_department.php?id='+row.id;
			...
			if (row){
				$.messager.confirm('Confirm','Are you sure you want to remove this department?',function(r){
					if (r){
						...
						$.post('<?php echo JURI::root( true ) ?>/newfiles/admin/department/delete_department.php',{id:row.id},function(s){
...
<body>
	...
	<table id="dg" title="My Users" style="width:900px;height:450px"
			url="<?php echo JURI::root( true ) ?>/newfiles/admin/department/get_departments.php"
