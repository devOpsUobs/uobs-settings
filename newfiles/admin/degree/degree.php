...
?>
	...
	<script type="text/javascript">
			...
			$('#dg').datagrid({
				...
				onExpandRow: function(index,row){
					...
					ddv.panel({
						border:false,
						cache:true,
						href:'<?php echo JURI::root( true ) ?>/newfiles/admin/degree/form_degree.php?index='+index,
						onLoad:function(){
		...
		function saveItem(index){
			...
			var url = row.isNewRecord ? '<?php echo JURI::root( true ) ?>/newfiles/admin/degree/save_degree.php' : '<?php echo JURI::root( true ) ?>/newfiles/admin/degree/update_degree.php?id='+row.id;
			...
			if (row){
				$.messager.confirm('Confirm','Are you sure you want to remove this department?',function(r){
					if (r){
						...
						$.post('<?php echo JURI::root( true ) ?>/newfiles/admin/degree/delete_degree.php',{id:row.id},function(s){
...
<body>
	...
	<div class="demo-info" style="margin-bottom:10px">
		...
		<div>Manage degrees</div>
	</div>
	
	
	<table id="dg" title="My Users" style="width:700px;height:500px"
			url="<?php echo JURI::root( true ) ?>/newfiles/admin/degree/get_degrees.php"
