...
<table class="table table-bordered table-striped">
	...
	<tr>
		...
		<td><?php $department = showEvalDepartmentSel($conn, $evl_id, $department); ?></td>
	</tr>
</table>
...
if ($evl_id > 0) {
	...
	<form id="fm1" method="post"
		...
		<input type="hidden" name="sel_dep_id" value="<?php echo $department ?>" />
	</form>
	...
	else
	{
		?>
		<form id="fm1" method="post"
			action="<?php echo JURI::root(true) ?>/newfiles/evaluation/eva_summary/teacher_eva_summary_old/msExcelExport.php"
			target="_blank">
			<input type="hidden" name="evl_id" value="<?php echo $evl_id ?>" />
			<input type="hidden" name="sel_dep_id" value="<?php echo $department ?>" />
		</form>
...
function showEvalDepartmentSel($conn, $evl_id, $dep)
{
	$db = JFactory::getDBO();
	$db->setQuery(
		$db->getQuery(true)
...
<input type="hidden" id="evl_id" name="evl_id" value="' . $evl_id . '"/>
