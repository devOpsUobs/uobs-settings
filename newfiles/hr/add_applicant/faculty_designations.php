...
if(isset($_POST['add_designations']) || isset($_POST['save_next']))
{
	...
	if($_POST['emp_desig_id'] != NULL)
	{
		...
		$emp_deg_cur=mysqli_query($conn, "SELECT * FROM `kiusc_emp_designations` where id='$desig_id'");
		$previous_desg_data=mysqli_fetch_assoc($emp_deg_cur);
	...
	{
		$chk_already_exists = mysqli_query($conn, "SELECT * FROM `kiusc_emp_designations` 
													WHERE `designation_id`='$designation_id' 
														AND `acc_department_id`='$acc_department_id' 
														AND `adm_department_id`='$adm_department_id'
														AND `pay_scale` = '$pay_scale'
														AND `employment_nature` = '$employment_nature'
														AND `employment_scale` = '$employment_scale'
														AND `pay_scale` = '$pay_scale'
														AND `date_of_joining` = '$date_of_joining'
														AND emp_id='$emp_id'");

		$chk_already_exist = mysqli_num_rows($chk_already_exists);
		if($chk_already_exist > 0)
		{
			echo "<font color='red'> This Record is Already Exists...!! </font>";
		}
		else
		{
			if($is_current == 1)
			...
			log_employee_change($conn, 'kiusc_employees', [], $current_desg_data, $user->id);
...
<tr>
	...
	<tr>
		...
		<td style="text-align:right">
			<input type="submit" name="add_designations" value="Add Designation" class="btn success" />
		</td>	
	</tr>
	<tr>
		<td>&nbsp;</td>
		...
		<tr>
			<td><input type="submit" name="save_next" value="Save & Next" class="btn success" /></td>
			<td>
...
<table class="table table-striped table-hover">
	...
	<?php
	$i=0;
	$desig = mysqli_query($conn, "SELECT ed.*, d.designation FROM `kiusc_emp_designations` ed JOIN kiusc_designations d on d.id=ed.designation_id where emp_id='$emp_id' ORDER BY ed.date_of_joining ASC");
		...
		<tr>
			...
			<td width="100px">
				<form action="" method="POST">
					...
					<?php
					if($desg['is_initial'] == 0)
					{
						?>
						<input type="submit" name="edit" value="Edit"  class="btn info" />
						<input type="submit" name="delete" value="Delete" class="btn danger" />
						<?php
					}
					else
					{
						echo "<span style='color:red'>Change from Initial Creation</span>";
					}
					?>
				</form>
			</td>
