				...
				foreach($days as $day)
					...
					//print_r($temp3_arr);
							...
							if ($conflict ==0 && $teacher==0 && $rom==0 && $prog_con==0)
							{
								...
								mysqli_query($conn,"UPDATE `kiusc_timetable` SET `room_id` = '$room',`start` = '$s', `end` = '$e', `day` = '$day', updated_by = '$user_id', updated_date = 'NOW()' where id =".$id);
...
<?php
    		...
    		echo '<input type="submit" value="Generate"  name ="generate" class="btn2 success"/>';
?>
    	</td>
	</tr> 
</table>
