...
if (isset($_POST['insert']))
{
	...
	if($change_room == 1)
	{
		mysqli_query($conn,"UPDATE `kiusc_timetable` SET room_id = '$room' where id = '$chours'");
		echo "UPDATE `kiusc_timetable` SET room_id = '$room' where id = '$chours'";
	}
	else
	{
	mysqli_query($conn,"UPDATE `kiusc_timetable` SET end_date = '$new_date', is_changed = 1 where id = '$chours'");
	
	mysqli_query($conn,"INSERT INTO `kiusc_timetable` (`id`, `sem_id`, `c_offer_id`, `room_id`, `fac_id`, `start`, `end`, `day`, `period`, `start_date`, `end_date`) 
	VALUES (NULL, '$sem', '$course', '$room', '$fac', '$s', '$e', '$day', '".$per['period']."', '$new_date', '". $date['final_term_date'] ."')");
	}
	$conflict=1;
	$teacher=1;
	$rom=1;
