...
if (isset($_POST['insert']))
{
	...
	mysqli_query($conn,"UPDATE `kiusc_timetable` SET `room_id` = '$room',`start` = '$s', `end` = '$e', `day` = '$day', updated_by = '$user_id', updated_date = 'NOW()' where id = '$chours'");
	
	/*
		$result = mysqli_query($conn,"select * from kiusc_timetable where id = '$chours'");
		...
		if($check_day == $day)
		{
			...
			for($i = $sdate; $i <=$edate; $i = date_modify($i,"+1 week") )
			{
				...
				mysqli_query($conn,"INSERT INTO `kiusc_lectures` (`id`, `date`, `c_offer_id`, `period_id`) VALUES (NULL, '$ins', '$course', '$chours')");	
			}
...
if (isset($_POST['check']))
{
	...
	$temp_arr = StudentConflict($conn, $sem, $course, $s_time, $e_time, $day);
	$conflict_arr = $temp_arr['con_arr'];
	$conflict = $temp_arr['is_con'];
	
	if($merge_cf_id != 0)
	{
		$temp_arr = StudentConflict($conn, $sem, $merge_cf_id, $s_time, $e_time, $day);
	...
	$temp2_arr = TeacherConflict($conn, $sem, $course, $fac, $s_time, $e_time, $day);
	$teach_arr = $temp2_arr['con_ar'];
	$teacher = $temp2_arr['is_con'];
	
	$temp3_arr = RoomtConflict($conn, $sem, $course, $room, $s_time, $e_time, $day);
	...
	if($merge_cf_id != 0)
	{
		...
		$temp4_arr = programConflict($conn, $sem, $prog2['prog_id'], $s_time, $e_time, $day);
...
echo '
	<tr>
		<td> Select Periods : </td>';
			$b='';
			
?>
			<td>
<?php
			$chours = showC_hoursSel($conn, $chours,$room,$course, $sem, $prog, $dept);
			$per=mysqli_query($conn, "select * from kiusc_timetable where id = '$chours'");
			$per=mysqli_fetch_assoc($per);
		 	if($per['start']!=0)
			{
				//$a = "style='background-color:#D2933C'";
...
<?php
		...
		if($rom==0 && $teacher==0 && $per['start']==0 && $prog_con==0)
		{
...
{
	$chours_re =  mysqli_query($conn,"select * from kiusc_timetable where c_offer_id = '$course' and is_changed=0");
