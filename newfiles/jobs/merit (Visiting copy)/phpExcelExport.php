...
$sql = "INSERT INTO `kiusc_job_result`(`app_id`, `name`, `scc_total`, `scc_obt`, `scc_per`, `scc_points`, `hssc_total`, `hssc_obt`, `hssc_per`, `hssc_points`, `bachelor_total`, `bachelor_obt`, `bachelor_per`, `bachelor_points`, `master_total`, `master_obt`, `master_per`, `master_points`, `mphil_points`, `phd_points`, `publications`, `experience`, `total_accademics`, `accd_weightage`, `total_interview`, `g_total`, `remarks`) VALUES ("; 
...
while($app = mysqli_fetch_assoc($applicant))
{
	...
	$sql_v .=  "'". ($view['percentage'] * 1) .  "',";
	
	$t_accd += $view['percentage'] * 1;
	...
	$sql_v .=  "'". ($view['percentage'] * 2) . "',";
	
	$t_accd += $view['percentage'] * 2;
	...
	$sql_v .=  "'". ($view['percentage'] * 3 ) . "',";
	
	$t_accd += $view['percentage'] * 3;
	...
	$sql_v .=  "'". ($view['percentage'] * 4) . "',";
	
	$t_accd += $view['percentage'] * 4;
...
while($row = mysqli_fetch_assoc($re))
{
	...
	$objPHPExcel->getActiveSheet()->setcellValue('K'.$base, $row['total_accademics']);
	
	$objPHPExcel->getActiveSheet()->setcellValue('L'.$base, $row['accd_weightage']);
	
	$objPHPExcel->getActiveSheet()->setcellValue('M'.$base, $row['mphil_points']);
	
	$objPHPExcel->getActiveSheet()->setcellValue('N'.$base, $row['experience']);
